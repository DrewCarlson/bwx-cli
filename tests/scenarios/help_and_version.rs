//! `rbw --help` and `rbw --version` are the first thing a user runs to
...
fn help_and_version_work_without_agent() {
    ...
    assert!(version.status.success(), "rbw --version failed");
    let ver_out = String::from_utf8_lossy(&version.stdout);
    // Expect something of the form "rbw 1.15.0" (semver-ish).
    assert!(
        ver_out.to_lowercase().contains("rbw"),
        "--version output doesn't mention rbw: {ver_out:?}"
    );
    assert!(
    ...
    assert!(help.status.success(), "rbw --help failed");
    ...
    // `rbw help <subcommand>` is equivalent and should also work.
    let get_help = harness.run(&["help", "get"]);
    assert!(get_help.status.success(), "rbw help get failed");
}
